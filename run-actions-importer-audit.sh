...
# run-actions-importer-audit.sh
#
# Runs a GitHub Actions Importer audit (via the actions-importer Docker image,
# no local `gh` CLI extension required) across multiple Azure DevOps
# organizations.
#
# Usage:
#   ./run-actions-importer-audit.sh [-c <csv-file>] [-o <output-dir>] [-s] [-h]
#
# Environment:
...
#
# Output layout:
#   <output-dir>/<organization>/         Full audit output for that org.
#   <output-dir>/meta/<org>_audit_summary.md
#   <output-dir>/meta/<org>_workflow_usage.csv
#     After each successful audit, audit_summary.md and workflow_usage.csv
...
#     2. Run the whole script with sudo: `sudo ./run-actions-importer-audit.sh`.
...
#   -o, --output-dir <dir>    Base output directory for audit results
#                             (default: output)
...
# Allocate a pseudo-TTY for the `docker run` audit calls when stdout is
...
# clear guidance, rather than letting every per-org audit fail one by one.
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}GitHub Actions Importer Audit Runner${NC}"
echo -e "${BLUE}========================================${NC}"
...
while IFS=',' read -r org custom_url pat; do
    ...
    # Run the audit via the actions-importer Docker image directly.
    ...
    if $DOCKER_CMD run --rm $DOCKER_TTY_FLAG \
        ...
        echo -e "${GREEN}✓ Audit completed for $org${NC}"
        ...
        for summary_file in audit_summary.md workflow_usage.csv; do
        ...
        echo -e "${RED}✗ Audit failed for $org${NC}"
...
# Print summary
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Audit Run Summary${NC}"
echo -e "${BLUE}========================================${NC}"
...
if [[ $FAILED_ORGS -eq 0 ]] && [[ $TOTAL_ORGS -gt 0 ]]; then
    ...
    echo -e "${YELLOW}Some audits failed. Check output above for details.${NC}"
    exit 1
fi
